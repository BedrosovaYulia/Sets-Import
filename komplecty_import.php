	...
	function onAfterProductUpdate(&$arFields)
	{
		...
		{//Если инфоблок является торговым каталогом
			...
			if (isset($CatalogTrPrID)){//если существует свойство с реквизитами
				...
				//Посмотрим его инфоблок торговых предложений
				$catalog_info=CCatalog::GetByID($CatalogID);
				$IB_SKU=$catalog_info['OFFERS_IBLOCK_ID'];
					...
					foreach ($arFields['PROPERTY_VALUES'][$CatalogTrPrID] as $trait){
						...
						if ($trait['DESCRIPTION']=='Комплектующее'){
							...
							//die();
								...
								//Ищем это торговое предложение
								$arSelect = Array("ID", "NAME");
								$arFilter = Array("IBLOCK_ID"=>$IB_SKU, "XML_ID"=>$p_xml_id.'#'.$p_tp);
								...
								}//конец если найдено торговое предложение

							else{


								//Ищем ID товара по его XML_ID
								$arSelect = Array("ID", "NAME");
						...
						}//конец иначе если не найдено торговое прдложение
